...
I/O 修订的同型号 32-instance、100 轮试跑已完成：无审计 6.294 秒，完整记录 15.852 秒，
比值 2.518；相较修订前的记录 wall time 下降约 30.6%。两次试跑在同型号上分别配对，
不是多次重复的稳健计时估计。路径及 anytime 仍完全一致。
后台写入累计降至 7.93 秒，几何数据仅存一份。尚未达到 5% 开销目标。
原 32-instance 的 AS/MMAS 分块与重排检查均已通过。A4000 尚无空闲卡，继续由 nohup 监控等待。
