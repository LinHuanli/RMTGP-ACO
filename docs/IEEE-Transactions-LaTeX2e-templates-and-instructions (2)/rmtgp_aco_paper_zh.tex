...
\begin{table*}[t]
\caption{状态转移终端。全部输出广播为 \texttt{TrField}。}
\label{tab:transition-terminals-zh}
\centering
\footnotesize
\renewcommand{\arraystretch}{1.18}
\begin{tabular}{L{21mm}L{54mm}L{72mm}L{17mm}}
\toprule
终端 & 计算方法 & 直观含义 & 配置\\
\midrule
\texttt{RTau} &
...
\begin{table*}[t]
\caption{信息素终端。全部输出为 \texttt{PhField}。}
...
保护除法定义为
\(\operatorname{PDIV}(x,y)=xy/(y^2+10^{-6})\)。
它在 \(y=0\) 附近连续。
每次函数计算后，NaN 被替换为零。
正负无穷被替换为 \(10\) 和 \(-10\)。
其余结果也截断到 \([-10,10]\)。
...
\caption{TSP100 训练与验证曲线。曲线为三个独立 GP 运行的中位数。阴影为最小值和最大值。每代使用新的训练批量。图内坐标保留英文。}
...
\caption{Full-F1 与 baseline ACO 的参考差距之差。误差线为分层 bootstrap 95\% 区间。负值支持 Full-F1。图内坐标保留英文。}
