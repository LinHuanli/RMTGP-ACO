...
\begin{table*}[t]
\caption{状态转移终端的定义与强类型。全部终端的返回类型为 \texttt{TrField}。}
\label{tab:transition-terminals-zh}
\centering
\footnotesize
\renewcommand{\arraystretch}{1.18}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{L{22mm}L{22mm}L{51mm}L{56mm}L{15mm}}
\toprule
终端 & 输出 Type & 计算方法 & 直观含义 & 配置\\
\midrule
\texttt{RTau} &
\texttt{TrField} &
对当前可行边的 \(\log(\max(\tau,\epsilon))\) 做标准化，再取 \(\tanh\) &
候选边相对于当前可行集合的信息素强弱 &
Core、Full\\
\texttt{REta} &
\texttt{TrField} &
对当前可行边的 \(\log(\max(\eta,\epsilon))\) 做标准化，再取 \(\tanh\) &
候选边相对于当前可行集合的距离启发式强弱 &
Core、Full\\
\texttt{BaseConf} &
\texttt{TrField} &
计算 \(\tanh[\log(p^0+\epsilon)+\log k]\) &
原始分布对该候选的相对信心。均匀分布时为零 &
Core、Full\\
\texttt{DistRank} &
\texttt{TrField} &
距离名次为 \(r\) 时，计算 \(1-2(r-1)/(k-1)\)；\(k=1\) 时设零 &
最近城市映射为 1，最远城市映射为 \(-1\) &
Core、Full\\
\texttt{Entropy} &
\texttt{TrField} &
计算 \(2H(p^0)/\log k-1\)，并广播到候选；\(k=1\) 时设为 \(-1\) &
原始转移分布的不确定性 &
Full\\
\texttt{ConstructProg} &
\texttt{TrField} &
...
\begin{table*}[t]
\caption{信息素终端的定义与强类型。全部终端的返回类型为 \texttt{PhField}。}
...
\texttt{EdgeEta} &
\texttt{PhField} &
比较边 \((u,v)\) 的对数启发式与两个端点邻域的平均值，再在来源路线内标准化 &
该边相对于两个端点局部邻域的距离质量 &
Core、Full\\
\texttt{EdgeTau} &
\texttt{PhField} &
对来源路线内的 \(\log(\max(\tau_{uv},\epsilon))\) 做标准化 &
该边相对于同一增强事件中其他边的信息素强弱 &
Core、Full\\
\texttt{NNRank} &
\texttt{PhField} &
将 \(u\!\to\!v\) 与 \(v\!\to\!u\) 的最近邻名次映射到 \([-1,1]\)，然后取平均 &
无方向的局部邻近程度 &
Core、Full\\
\texttt{ColonyFreq} &
\texttt{PhField} &
边在当前蚁群中出现 \(c_e\) 次时，计算 \(2c_e/M-1\) &
当前蚁群对该边的共同选择程度 &
Full\\
\texttt{SourceQuality} &
\texttt{PhField} &
计算 \(\tanh[(\overline L-L_r)/(\sigma_L+\epsilon)]\)，并广播到路线边 &
来源路线相对于当前蚁群的质量 &
Core、Full\\
\texttt{ACOProg} &
\texttt{PhField} &
...
RQ2 & 残差状态转移是否优于直接替换完整状态转移？ & TR-RGP 与 Matched-Replace & 待完成\\
RQ3 & 相同总节点预算下，双树是否优于单树？ & Full-F1 与独立训练的 TR-RGP、PH-RGP & 待完成\\
RQ4 & 改进是否依赖 Full 终端或 F1 函数？ & Core/Full 与 F0/F1 的 \(2\times2\) 因子实验 & 待完成\\
RQ5 & 两棵树之间是否存在联合适应？ & 删除一棵树，以及跨 GP 运行打乱树配对 & 待完成\\
RQ6 & TSP100 训练的程序能否迁移到其他规模和分布？ & TSP50/500/1000，以及 cluster、Gaussian、TSPLIB & 规模已完成\\
...
待测 & TSP500 cluster / Gaussian & \(128/128\)\\
待测 & TSPLIB，\(n\leq500\) & 42\\
\bottomrule
\end{tabular}
\end{table}
...
Legacy-GP & 完整替换 & 不修改 & 上一研究配置 & 在当前预算下复现上一篇 GP-ACO & \pendingresult\\
Matched-Replace & 完整替换 & 不修改 & Full/F1 & RQ2 的匹配替换对照 & \pendingresult\\
TR-RGP & 残差 & 不修改 & Full/F1 & 独立训练的状态转移单树 & \pendingresult\\
PH-RGP & 不修改 & 预算残差 & Full/F1 & 独立训练的信息素单树 & \pendingresult\\
Core-F0 & 残差 & 预算残差 & Core/F0 & RQ4 因子实验 & \pendingresult\\
Core-F1 & 残差 & 预算残差 & Core/F1 & RQ4 因子实验 & \pendingresult\\
Full-F0 & 残差 & 预算残差 & Full/F0 & RQ4 因子实验 & \pendingresult\\
Full-F1 & 残差 & 预算残差 & Full/F1 & 本文主方法 & uniform 已完成\\
Drop / Shuffle & 训练后干预 & 训练后干预 & 固定 Full/F1 树 & RQ5 联合适应分析 & \pendingresult\\
\bottomrule
\end{tabular}
\end{table*}
...
\pendingresult。
主要比较是 TR-RGP 与 Matched-Replace。
两者都使用一棵状态转移树、Full 终端、F1 函数和相同节点预算。
唯一差异是树输出的接入方式。
TR-RGP 修正原始分数。
Matched-Replace 直接生成完整分数。

Legacy-GP 是辅助基线。
它同时改变终端和函数。
因此，Legacy-GP 不能单独识别 residual 的作用。

\textbf{RQ2 的当前答案：}
目前不能说明 residual 比完整替换更好。
Full-F1 优于 baseline ACO 不能回答这个问题。

\subsection{RQ3：双树与单树}

\pendingresult。
主要比较包含 Full-F1、独立训练的 TR-RGP 和独立训练的 PH-RGP。
三者共享 31 个总有效节点。
双树的 31 个节点在两棵树之间分配。
它不是每棵树各 31 个节点。
这种设置控制了总语法容量。

31 与 62 节点的容量实验用于检查结果是否依赖预算。
训练后删除一棵树只能分析已有个体的机制。
它不能替代独立训练的单树对照。
...
Full 与 Core 的平均差异表示终端主效应。
F1 与 F0 的平均差异表示函数主效应。
两种差异是否相互依赖由交互效应判断。

Core 保留四个状态转移终端和四个信息素终端。
Full 增加进度、熵、停滞和蚁群频率信息。
F1 增加 \texttt{MIN}、\texttt{MAX} 和 \texttt{ABS}。
该实验可以识别没有必要的输入或函数。
...
第一项干预将状态转移树替换为零。
第二项干预将信息素树替换为零。
第三项干预在独立 GP 运行之间打乱两棵树的配对。
若打乱后性能下降，则说明存在配对相关的联合适应。
该结果仍不能代替 RQ3 的独立单树训练。
...
该结果不能证明分布迁移。
Cluster TSP500、Gaussian TSP500 和 TSPLIB 的结果仍待完成。
...
RQ1 结果不能提供因果解释。
Residual 与替换、双树与单树、函数与终端配置、联合适应都需要各自的受控实验。
