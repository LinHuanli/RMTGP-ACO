...
RQ1、RQ2、RQ4--RQ6 已有完整的三种子先导结果。
RQ3 的 31 节点比较已经完成。
31/62 节点容量敏感性仍需单独报告。
RQ7 的融合训练基准已经完成，单程序部署计时仍在运行。
...
RQ7 & 批量融合实现能提供多大加速？ & CPU8、CPU16、单 GPU、双 GPU 与并行 campaign &
训练已完成；部署计时中\\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{table}[!t]
\caption{Residual 与完整替换的配对差值。
单元格为均值差 [95\% 分层 bootstrap 区间]，单位为百分点。
负值表示 residual 更好。}
\label{tab:residual-effects-zh}
\centering
\scriptsize
\setlength{\tabcolsep}{2.5pt}
...
\caption{Residual 接口与完整替换的配对差值。
误差线为 95\% 分层 bootstrap 区间。
负值支持 residual。图内坐标保留英文。}
\label{fig:residual-effects-zh}
\end{figure}

表~\ref{tab:residual-effects-zh} 和
图~\ref{fig:residual-effects-zh} 给出本 RQ 的直接 estimand。
...
\begin{table}[!t]
\caption{固定 31 个总节点时，双树与独立训练单树的配对差值。
单元格为均值差 [95\% 分层 bootstrap 区间]，单位为百分点。
负值表示双树更好。}
\label{tab:architecture-b31-zh}
\centering
\scriptsize
\setlength{\tabcolsep}{2.5pt}
\resizebox{\columnwidth}{!}{%
...
\caption{固定 31 个总节点时，双树与两种独立训练单树的配对差值。
误差线为 95\% 分层 bootstrap 区间。
负值支持双树。图内坐标保留英文。}
\label{fig:architecture-b31-zh}
\end{figure}

表~\ref{tab:architecture-b31-zh} 和
图~\ref{fig:architecture-b31-zh} 比较 Full-F1、TR-RGP 和 PH-RGP。
三种方法都独立训练。
三者共享 31 个总有效节点。
双树的 31 个节点在两棵树之间分配。
它不是每棵树各 31 个节点。
...
当前 AS、ACS 和 MMAS 的 TSP50/TSP100 已完成。
ACS 的 TSP500 也已完成。
七个已完成单元格的中位开销为 \(+53.83\%\) 至 \(+487.25\%\)。
AS 和 MMAS 的已完成开销高于 ACS。
这说明训练吞吐量提高不等于单程序部署没有额外成本。
表中的“进行中”不是零开销，也不进入当前汇总。
全部计时完成后，生成脚本会用冻结报告自动替换这些单元格。

\textbf{RQ7 的当前答案：}
“个体 \(\times\) 实例”融合使正式 ACO 预算下的 GP 训练可行。
多 GPU 环境应优先并行独立 GP 运行。
训练吞吐量数值只适用于登记的 RTX 4000 Ada 工作负载。
孤立部署开销只适用于当前 RTX A5000 测量。
其他硬件需要重新测量。
